...
void spanning_subtree_algorithm(const Param* const param,
  ...
  // for (int i = 0; i < 10; i++) {
  //   printf(
  //       "i=%d (%d, %d): %d\n", i, sorted_edge_idxs[i][0],
  //       sorted_edge_idxs[i][1],
  //       weighted_adjacency_mat[sorted_edge_idxs[i][0]][sorted_edge_idxs[i][1]]);
  //   printf("%d\n", dist(sorted_edge_idxs[i][0], sorted_edge_idxs[i][1]));
  // }
  printf("[INFO] endup sort_edge_idxs(time=%f)\n", cpu_time());
#endif
...
#if DEBUG
  printf("[INFO] endup compute_min_upper_spanning_tree_kruskals(time=%f)\n",
         cpu_time());
#endif
  ...
  two_opt(tspdata->n, tspdata->min_node_num,
          (param->timelim - cpu_time() - vdata->starttime) * 0.2, tspdata->x,
...
void compute_min_spanning_tree_kruskals(const int n_nodes,
  ...
  bool out_of_use_mat[n_nodes][n_nodes];
  for (int i = 0; i < n_nodes; i++) {
  ...
  for (int prune_idx = 0; prune_idx < n_prune_nodes; prune_idx++) {
    ...
    for (int temp_prune_node = 0; temp_prune_node < n_nodes;
      ...
      for (int temp_connected_node = 0; temp_connected_node < n_nodes;
           temp_connected_node++) {
        if (!spanning_tree_adjacency_mat[temp_prune_node]
                                        [temp_connected_node]) {
          continue;
        } else if (n_branch > 1) {
          break;
...
void get_descendants(const int parent_node,
  ...
  for (int temp_child_node = 0; temp_child_node < n_nodes; temp_child_node++) {
    if (!adjacency_mat[parent_node][temp_child_node]) {
      continue;
    }
